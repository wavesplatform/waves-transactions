...
import { IOrder, TOrder, } from './transactions'
...
const getIssueData = (t: TIssueTransaction): wavesProto.waves.IIssueTransactionData => ({
    name: t.name,
...
const getTransferData = (t: TTransferTransaction): wavesProto.waves.ITransferTransactionData => ({
    recipient: recipientToProto(t.recipient),
    amount: amountToProto(t.amount, t.assetId),
    attachment: attachmentToProto(t.attachment)
})
const getReissueData = (t: TReissueTransaction): wavesProto.waves.IReissueTransactionData => ({
...
const getCancelLeaseData = (t: TCancelLeaseTransaction): wavesProto.waves.ILeaseCancelTransactionData => ({
    leaseId: base58Decode(t.leaseId)
})
const getAliasData = (t: TAliasTransaction): wavesProto.waves.ICreateAliasTransactionData => ({alias: t.alias})
const getMassTransferData = (t: TMassTransferTransaction): wavesProto.waves.IMassTransferTransactionData => ({
    assetId: t.assetId == null ? null : base58Decode(t.assetId),
    attachment: attachmentToProto(t.attachment),
    transfers: t.transfers.map(massTransferItemToProto)
})
const getDataTxData = (t: TDataTransaction): wavesProto.waves.IDataTransactionData => ({
    data: t.data.map(dataEntryToProto)
})
const getSetScriptData = (t: TSetScriptTransaction): wavesProto.waves.ISetScriptTransactionData => ({
    script: t.script == null ? null : scriptToProto(t.script)
})
const getSponsorData = (t: TSponsorshipTransaction): wavesProto.waves.ISponsorFeeTransactionData => ({
    minFee: amountToProto(t.minSponsoredAssetFee, t.assetId)
})
const getSetAssetScriptData = (t: TSetAssetScriptTransaction): wavesProto.waves.ISetAssetScriptTransactionData => ({
    assetId: base58Decode(t.assetId),
    script: t.script == null ? null : scriptToProto(t.script)
})
const getInvokeData = (t: TInvokeScriptTransaction): wavesProto.waves.IInvokeScriptTransactionData => {
...
const getUpdateAssetInfoData = (t: TUpdateAssetInfoTransaction): wavesProto.waves.IUpdateAssetInfoTransactionData => {
    return {
        assetId: base58Decode(t.assetId),
        name: t.name,
        description: t.description
...
export const txToProto = (t: TTransaction): wavesProto.waves.ITransaction => {
...
export const dataEntryToProto = (de: TDataTransactionEntry | TDataTransactionDeleteRequest): wavesProto.waves.DataTransactionData.IDataEntry => ({
    key: de.key,
